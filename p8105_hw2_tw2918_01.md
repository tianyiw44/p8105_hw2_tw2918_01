...
- used `rename` function to rename study_id to id to make it consistant
  with `baseline_df`
- There are 487 participants in this dataset
- There are 6variable including the `baseline` time in years when
  biomaker amyloid is measured, the second visit (`time_2`), fourth
  visit (`time_4`), sixth visit (`time_6`) and Eightth visit (`time_8`)
  when it is measured.
